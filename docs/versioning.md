...
Existing packages with the previous minor version remain valid if they omit the new fields. Bumping `specVersion` to the new minor is recommended for new work.
...
Spec 1.1.0 added optional fields only. No breaking changes. Existing 1.0.0 packages validate without modification.

**Recommended updates when bumping to 1.1.0:**

| Area | Change |
|------|--------|
| Landing page | Add `{ "id": "screenshots", "enabled": true, "source": "media" }` if you have screenshots |
| Media | Add optional `title` and `description` on `media.screenshots[]` entries |
| Mockup | Add `installCommand` and `devCommand` if using a JS toolchain |
| Ads | Add `platforms[]`; migrate `utmTemplate` from string to structured object |
| Deployment | Add `vercelDeploymentUrl` and `lastDeployedAt` (leave `null` until automation runs) |
| App Store | Add `supportUrl` and `privacyPolicyUrl` if preparing store metadata |
...
```json
{
  ...
  "landingPage": {
    "sections": [
      { "id": "hero", "enabled": true, "source": "inline", "inline": { "headline": "..." } },
      { "id": "screenshots", "enabled": true, "source": "media" },
      { "id": "cta", "enabled": true, "source": "inline", "inline": { "headline": "..." } }
    ]
  },
  "ads": {
...
4. Update [docs/](.) supporting documentation
5. Add entry to [CHANGELOG.md](../CHANGELOG.md)
6. Tag release: `v1.1.0` (repo) matching spec `1.1.0`
...
- Workflows should accept both structured and string `ads.utmTemplate` forms until all packages migrate
...
The string form of `ads.utmTemplate` is deprecated in favor of the structured object as of 1.1.0 but remains valid.
